...
class GeminiService {
  Future<String> generateQuestionAndAnswers() async {
    ...
    try {
      ...
      final finalPrompt = getPrompt(chosenSubject);
      ...
      final decodedResponse = await response;
      final output = decodedResponse?.content?.parts?.last.text;
      // print(decodedResponse);
      // print(output);

      if (decodedResponse == null) {
        return "decodedResponse is null";
      } else {
        return output!;
      }

      // Legacy code w/ PaLM API
      ...
      return 'Error! Please inform Terris to fix this!!!';
    }
  }
}

String getPrompt(String chosenSubject) {
  var prompt =
      "You are a primary school teacher, teaching children of ages 6 to 12 years old. Your task is to create a question for a given subject. Format your response as follows: Question: <question> Options: <option1>, <option2>, <option3>, <option4> Answer: <answer> Explanation: <explanation>";
  prompt +=
      "Come up with a question for $chosenSubject. Format your response as follows: Question: <question> Options: <option1>, <option2>, <option3>, <option4> Answer: <answer> Explanation: <explanation>";
  prompt +=
      "These are some example references, of questions you can create for $chosenSubject: ";

  if (chosenSubject == "Mathematics") {
    prompt += mathExamples.join(';');
  } else if (chosenSubject == "Science") {
    prompt += scienceExamples.join(';');
  } else if (chosenSubject == "Art") {
    prompt += artExamples.join(';');
  } else if (chosenSubject == "Flutter") {
    prompt += flutterExamples.join(';');
  }

  prompt +=
      "Please format the question with proper next lines, spacing, and punctuation marks. No need to bold.";
